...
export default {
  ...
  methods: {
	...
	this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../ssm3t2mm/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `redianxinwen/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.redianxinwenCrossAddOrUpdateFlag = false;
                  this.parent.search();
